...
let AuthService = class AuthService {
    ...
    async signin(params) {
        ...
        if (!user) {
            throw new common_1.UnauthorizedException('User is wrong');
        }
        const checkPassword = await (0, bcrypt_1.compare)(params.password, user.password);
        if (!checkPassword) {
    ...
    async register(params) {
        ...
        if (checkUserName)
            throw new common_1.ConflictException('Username is alreadt exists');
